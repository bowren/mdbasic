.print "***MDBASIC***"
.print "by: Mark Bowren"
.print @"(c)1989 Bowren Consulting, Inc. (www.bowren.com)\n"

.label COLOR  = $0286 //Current Foreground Color for Text
.label HIBASE = $0288 //(648) Top Page of Screen Memory
.label SHFLAG = $028d //SHIFT/CTRL/Logo Keypress flags: Bit0: SHIFT, Bit1: Commodore Logo Key, Bit2: Ctrl Key

//Memory-Mapped I/O Registers
.label SP0X   = $d000 //Sprite 0 Horizontal Position
.label SP0Y   = $d001 //Sprite 0 Vertical Position
.label MSIGX  = $d010 //Most Significant Bits of Sprites 0-7 Horizontal Position
.label SCROLY = $d011 //Vertical Fine Scrolling and Control Register
/*
...
*/
.label LPENX  = $d013 //Light Pen Horizontal Position (0-160) must by multiplied by 2
.label LPENY  = $d014 //Light Pen Vertical Position (0-200) corresponds exactly to the current raster scan line.
.label SPENA  = $d015 //Sprite Enable Register - Bit 0:  Enable Sprite 0 (1=sprite is on, 0=sprite is off)
.label SCROLX = $d016 //Horizontal Fine Scrolling and Multicolor Control Register
/*
...
.label YXPAND = $d017 //Sprite Vertical Expansion Register - Bit 0-7 is Sprite 0-7 flag to expand sprite n vertically (1=double height, 0=normal height)

.label VMCSB  = $d018 //VIC-II Chip Memory Control Register
/*
 Bit 0: Unused,
 ...
 Bits 4-7: Video matrix base address within VIC-II 16K addressable memory
	...
	for each sprite.
*/
.label SPBGPR = $d01b //Sprite to Foreground Display Priority Register
.label SPMC   = $d01c //Sprite Multicolor Registers
.label EXTCOL = $d020 //Border Color Register
.label BGCOL0 = $d021 //Background Color 0 background color for text modes, sprites and mc bitmap. Default 6 (blue)
.label BGCOL1 = $d022 //Background Color 1 multicolor mode: bits 6 and 7 bit-pair 01 (screen codes  64-127) Default 1 (white)
.label BGCOL2 = $d023 //Background Color 2 multicolor mode: bits 6 and 7 bit-pair 10 (screen codes 128-191) Default 2 (red)
.label BGCOL3 = $d024 //Background Color 3 multicolor mode: bits 6 and 7 bit-pair 11 (screen codes 192-255) Default 3 (cyan)
.label SPMC0  = $d025 //Sprite Multicolor Register 0 (01 bit-pair)
.label SPMC1  = $d026 //Sprite Multicolor Register 1 (11 bit-pair)
.label SP0COL = $d027 //Sprite 0 Color Register (the default color value is 1, white)

//SID registers
.label FRELO1 = $d400 //Voice 1 Frequency Control (low byte
.label FREHI1 = $d401 //Voice 1 Frequency Control (high byte)
.label PWLO1  = $d402 //Voice 1 Pulse Waveform Width (low byte)
.label PWHI1  = $d403 //Voice 1 Pulse Waveform Width (high nybble)
.label VCREG1 = $d404 //Voice 1 Control Register
.label ATDCY1 = $d405 //Voice 1 Attack/Decay Register
.label SUREL1 = $d406 //Voice 1 Sustain/Release Control Register
.label CUTLO  = $d415 //Filter Cutoff Frequency (lo byte)
...
.label CHRGET = $0073 //Get Next BASIC Text Character
.label CHRGOT = $0079 //Get Current BASIC Text Character
.label KEYD   = $0277 //Keyboard Buffer (Queue)
...
.label FRMNUM2= $ad8d //validate numeric data type in FAC1
.label FRMEVL = $ad9e //evaluate expression
.label PARCHK = $aef1 //eval expr inside parentheses
.label CHKCLS = $aef7 //check for and skip closing parentheses
.label CHKOPN = $aefa //check for and skip opening parentheses 
.label CHKCOM = $aeff //check for and skip comma
.label PTRGET = $b08b //search for a variable & setup if not found
.label GIVAYF = $b391 //convert 16-bit signed integer to floating point (a=hibyte y=lobyte)
.label ERRDIR = $b3a6 //check if prg is running in direct mode/cause error
.label STRLIT = $b487 //scan and setup pointers to a string in memory
.label GETSPA = $b4f4 //alloc space in mem for string
.label FRESTR = $b6a3 //discard a temporary string
.label GETBYTC= $b79b //input a parameter whose value is between 0 and 255
.label GETADR = $b7f7 //convert FAC1 to unsigned 16-bit integer; result in $14 lobyte, $15 hibyte
.label OVERR  = $b97e //print overflow error message
.label FMULT  = $ba28 //multiply FAC1 by value in memory pointed to by A (lobyte) and Y (hibyte) registers
.label FDIVT  = $bb12 //divide FAC2 by FAC1: FAC1 = (FAC2/FAC1)
.label MOVFM  = $bba2 //move a floating point number from memory to FAC1
.label MOVEF  = $bc0f //copy FAC1 to FAC2 without rounding
.label ROUND  = $bc1b //round FAC1 to whole number
.label FINLOG = $bd7e //add signed integer to FAC1
.label FOUT   = $bddd //convert contents of FAC1 to ASCII String

//CBM BASIC error message display functions
.label SNERR  = $af08 //syntax error
.label BSERR  = $b245 //bad subscript error
.label FCERR  = $b248 //illegal quanity

//Commodore 64 Kernal functions
.label RESET  = $fce2 //power-on reset
.label SETMSG = $ff90 //set kernal msg ctrl flag
.label SETLFS = $ffba //set logical file parameters
.label SETNAM = $ffbd //set file name parameters
.label OPEN   = $ffc0 //open a logical file
.label CLOSE  = $ffc3 //close a logical file
.label CHKIN  = $ffc6 //define an input channel
.label CHKOUT = $ffc9 //define an output channel
.label CLRCHN = $ffcc //restore default devices
.label CHRIN  = $ffcf //input a character
.label CHROUT = $ffd2 //output a char to the current output device
.label LOAD   = $ffd5 //load from a device
.label STOP   = $ffe1 //test the stop key
.label CLALL  = $ffe7 //close all files
.label PLOT   = $fff0 //Read/Set Location of the Cursor

.label RESLST = $a09e //$A09E-$A19D List of Keywords
...
.label TOKEN_NEXT    = $82
.label TOKEN_INPUT_  = $84
.label TOKEN_INPUT   = $85
.label TOKEN_DIM     = $86
.label TOKEN_GOTO    = $89
.label TOKEN_RUN     = $8a
.label TOKEN_IF      = $8b
...
.label FIRST_CMD_TOK = $cb  //first MDBASIC token
.label TOKEN_OFF     = $cb  //OFF keyword token
.label TOKEN_ELSE    = $cc
.label TOKEN_VARS    = $cf
.label TOKEN_COLOR   = $d8
.label TOKEN_SPRITE  = $da
.label TOKEN_BITMAP  = $df
.label TOKEN_TEXT    = $e6
...
//
//token list that use line #'s that need to be renumbered when using renum
...
misop: .text "MISSING OPERAN"        //31
.byte 196
ilvne: .text "ILLEGAL VOICE NUMBE"   //32
.byte 210
illspr: .text "ILLEGAL SPRITE NUMBE" //33
.byte 210
illfun: .text "ILLEGAL FUNCTION CAL" //34
.byte 204
cantre: .text "CAN'T RESUM"          //35
.byte 197
//
...
prblok: jsr $bdd1 //output number to screen
 lda #$20
 jsr CHROUT
fprint: jsr CHRIN
 beq ename
...
clse7f: lda #$7f
 ...
 ldx #0         //designate default output channel (screen)
 jmp CHKOUT     //$b8 = current std out file number, 0=default (screen)
//
//*******************
//
...
newsave: pha
 lda $b9
 bmi newsav   //file handles >=128 are MDBASIC file handles
 pla
...
continu:
 ...
 lda $fc     //restore the address pointer on stack
...
// when foregndColor is > 15 flashing is turned on, otherwise off
color:
 cmp #','
 beq nochar
 jsr skip73
 ...
 jsr blink     //toggle blink flag for this color
 jsr chkcomm
nochar: jsr comck2
 beq noback
 jsr skip73
 sta BGCOL0     //back'gnd color
 jsr chkcomm
noback: jsr getval
...
//TODO: make a subroutine out of this to return base addr:
 ...
 ror
 ror
 sta $fc        //VIC-II Base Address hibyte: 0=$00, 1=$40, 2=$80 ,3=$C0
...
// lda $0d          //data type 0=numeric
// beq pfreq
// jmp nomtch       //mismatch error
//pfreq:
...
adsr: jsr ppw
 sta $fb       //SID register offset for voice
...
ppw2:
 ...
 dex         //0,1,2
 txa
 tay         //return voice index 0,1,2 in y reg
...
mcplot: jsr getval //get color selection, multicolor selection index 1-3
 beq badtype
 cmp #4         //mc mode color selection index is 1,2 or 3
 bcs badtype
...
loadd:
 ...
 pla
 ldx #0
 jsr lodsav  //get load param in $02
 jsr $f5af   //display Searching...
 jsr $f5d2   //display Loading...
 ldx $b8     //current logical file number
 jsr CHKIN   //define output channel
 lda $02     //original logical file number
 cmp #128    //file number 128 is for loading screens
 bne sdnot2
lodsrn: jsr param //prepare text and color mem pointers
 jsr CHRIN   //read first byte
 sta EXTCOL  //border color
 jsr CHRIN   //read second byte
 sta BGCOL0  //background color for all text modes
 jsr CHRIN
...
lode: jsr CHRIN //continue loading the rest of the data
 ...
 inc $c4
 bne lode
sdnot2: cmp #129 //file number 129 is for loading char design
 bne sdnot3
 jsr param2
lodchr: jsr CHRIN
 sta ($c3),y
 ...
 bne lodchr
sdnot3: jsr param3
 jsr CHRIN     //get multicolor flag
 sta SCROLX    //bit 4 is flag for multicolor mode (text or bitmap)
lodbm: jsr CHRIN
 sta ($c3),y
 jsr status    //check for stop key or EOF and do not return here if so
 inc $c3
...
savee: pla
 ...
 ldx $b8    //current logical file number
 jsr CHKOUT
 lda $02    //original file number
 cmp #128   //file number 128 is for saving screen
 bne sdvn2
savscr: jsr param
 lda $c4
 clc
...
srnend: lda $c4
 ...
 bne saves
 jmp romin
sdvn2: cmp #129 //file number 128 is for char design
 bne savbm
...
savbm: jsr param3  //prepare pointers for bitmap and color mem
...
lodsav: lda $b9  //current secondary address
 sta $02         //temp var
 stx $b9         //set current logical file number
 lda #$7f
 sta $b8         //set new logical file number
 jsr OPEN        //perform OPEN
...
cpybuf: lda ($fb),y
 ...
 lda ($fd),y
 sta bufcolor,y //color mem buffer
 dey 
 bpl cpybuf
 rts
//
